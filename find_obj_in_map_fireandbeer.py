...
class env_detect:
    ...
    def callback(self, rgb_img_msg, dep_img_msg, laser_sub):
        ...
        # if return False, continue attempting
        # if return 'failed', calling node movement code
        
        self.sing_res = self.beacon_control('fire_hydrant', laser_sub)
        if self.sing_res == 'failed':
            ...
            # remeber reset self.sing_res to False once the node is reached
        if self.sing_res:
            ...
            # let user input perferred objective
            # reset self.sing_res to False before next beaconing mission start
